      ...
      <div className="app-content flex flex-col gap-6">
        ...
        <div className="panel flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h3 className="section-title" style={{ fontSize: '1.25rem' }}>My Assets</h3>
          ...
          <div className="flex flex-col gap-3">
            {balances.length > 0 ? balances.filter((b: any) => {
              ...
              else if (code === 'EURT') { logoContent = <div className="w-full h-full rounded-full flex items-center justify-center font-bold text-[10px]" style={{ background: 'rgba(59,130,246,0.15)', color: 'var(--accent-primary)' }}>€</div>; }
              else { logoContent = <div className="w-full h-full rounded-full flex items-center justify-center font-bold text-[10px]" style={{ background: 'var(--background-elevated)', color: 'var(--foreground-secondary)' }}>{code?.slice(0, 3)}</div>; }
              return (
                <div key={i} className="flex items-center justify-between p-4 rounded-xl transition-colors" style={{ background: 'rgba(30,41,59,0.3)', border: '1px solid rgba(30,41,59,0.6)' }}>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 flex items-center justify-center shrink-0 overflow-hidden" style={{ border: '1px solid var(--border-default)', background: 'var(--background-card)', borderRadius: '9999px' }}>{logoContent}</div>
                    <div className="flex flex-col">
                      <span style={{ color: 'var(--foreground-primary)', fontWeight: 700, fontSize: '0.875rem' }}>{code}</span>
                      {b.asset_issuer && <span className="text-[10px] font-mono text-slate-400">Issuer: {b.asset_issuer.slice(0, 8)}...</span>}
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span style={{ color: 'var(--foreground-primary)', fontWeight: 700, fontSize: '0.875rem' }}>{balanceVal.toLocaleString(undefined, { maximumFractionDigits: 7 })}</span>
              ...
              <div className="flex flex-col items-center py-8 text-center">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'var(--background-elevated)' }}><Loader2 className="w-6 h-6 text-slate-400" /></div>
        ...
        {/* Available Assets */}
        <div className="panel flex flex-col">
          <h3 className="section-title" style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>Available Assets</h3>
          {error && <div className="text-sm font-medium p-4 rounded-xl mb-4" style={{ background: 'rgba(248,113,113,0.15)', border: '1px solid rgba(248,113,113,0.3)', color: 'var(--error)' }}>{error}</div>}
          {success && <div className="text-sm font-medium p-4 rounded-xl mb-4 flex items-center gap-2" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', color: 'var(--success)' }}><Check className="w-4 h-4" /> {success}</div>}
          <div className="flex flex-col gap-3">
            {COMMON_ASSETS.map((asset) => {
              const alreadyAdded = existingCodes.includes(asset.code);
              const isLoading = addingAsset === asset.code;
              return (
                <div key={asset.code} className="flex items-center justify-between p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.3)', border: '1px solid rgba(30,41,59,0.6)' }}>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 flex items-center justify-center shrink-0 overflow-hidden" style={{ border: '1px solid var(--border-default)', background: 'var(--background-card)', borderRadius: '9999px' }}>
                      ...
                      : asset.code === 'EURT' ? <div className="w-full h-full rounded-full flex items-center justify-center font-bold text-[10px]" style={{ background: 'rgba(59,130,246,0.15)', color: 'var(--accent-primary)' }}>€</div>
                    ...
                    <div className="flex flex-col"><span style={{ color: 'var(--foreground-primary)', fontWeight: 700, fontSize: '0.875rem' }}>{asset.code}</span><span className="text-[11px] text-slate-500 font-medium">{asset.name}</span></div>
                  </div>
                  {alreadyAdded ? (
                    <span className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg" style={{ color: 'var(--success)', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)' }}><Check className="w-3.5 h-3.5" /> Active</span>
                  ) : (
          ...
          <div className="mt-6 pt-6 border-t" style={{ borderColor: 'var(--border-default)' }}>
            <h4 className="font-extrabold text-sm mb-4" style={{ color: 'var(--foreground-primary)' }}>Custom Asset</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Asset Code</label>
                <input value={customCode} onChange={e => setCustomCode(e.target.value.toUpperCase())} placeholder="e.g. RANDOM" className="w-full p-3 rounded-xl text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" style={{ background: 'var(--background-elevated)', border: '1px solid var(--border-default)', color: 'var(--foreground-primary)' }} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Issuer Public Key</label>
                <input value={customIssuer} onChange={e => setCustomIssuer(e.target.value)} placeholder="G..." className="w-full p-3 rounded-xl text-sm font-medium font-mono focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" style={{ background: 'var(--background-elevated)', border: '1px solid var(--border-default)', color: 'var(--foreground-primary)' }} />
              </div>
            </div>
