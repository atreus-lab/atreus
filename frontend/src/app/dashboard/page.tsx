...
export default function DashboardPage() {
 ...
 <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-4 border-white ${isSend ? 'bg-orange-50 text-orange-500' : 'bg-green-50 text-green-500'}`}>
 {isSend ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownLeft className="w-4 h-4" />}
 </div>
 <div className="flex flex-col flex-1 pt-1">
 <div className="flex items-center justify-between">
 <span className="font-bold text-slate-900 text-sm">{isSend ? "Sent XLM" : "Received XLM"}</span>
 <span className="text-[11px] font-semibold text-slate-400 ">{new Date(tx.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
 </div>
 <span className={`text-[12px] font-semibold ${isSend ? 'text-slate-500 ' : 'text-green-600'}`}>
 {isSend ? '-' : '+'}{parseFloat(tx.amount).toFixed(2)} {tx.asset_code || "XLM"} {isSend ? "to" : "from"} {isSend ? tx.to?.slice(0, 5) : tx.from?.slice(0, 5)}...
 </span>
 </div>
 </div>
 );
 ...
 <span className="text-[12px] font-semibold text-slate-500 ">Your transactions will appear here.</span>
 </div>
 </div>
 )}
 </div>
