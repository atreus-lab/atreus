...
import { Loader2, ExternalLink, Trash2, Eye, EyeOff, LogIn, Copy, Check, ArrowRight, Lock, Shield, CheckCircle2, Key, Database, Layers, ArrowLeft, ShieldCheck, ChevronRight } from "lucide-react";
      ...
      <div className="w-full max-w-3xl">
        ...
        {numericBalance > 0 && (
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl px-6 py-4 mb-4 flex items-center justify-between shadow-[0_8px_20px_rgba(37,99,235,0.25)]">
            ...
            <span className="text-white font-extrabold text-2xl">{numericBalance.toFixed(2)} <span className="text-base opacity-80">XLM</span></span>
          </div>
        )}
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 mb-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {wallet.mnemonic.split(" ").map((word, i) => (
              <div key={`secure-word-${word}-${i}`} className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 flex flex-col items-center justify-center gap-1 hover:bg-slate-100 transition-colors">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 mb-6">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <div key={item.title} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-slate-50 transition-colors">
    ...
    <div className="min-h-screen bg-[#FAFBFF] text-slate-900 font-sans flex flex-col items-center px-4 py-10 relative">
      ...
      <div className="w-full max-w-2xl">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 mb-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button type="button" onClick={copyAddress} className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors">
              {addressCopied ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4 text-slate-500" />}
            </button>
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-3xl p-8 mb-4 text-center shadow-[0_12px_40px_-10px_rgba(37,99,235,0.4)] relative overflow-hidden">
          ...
          <p className="text-5xl font-black text-white tracking-tight relative z-10">
            {numericBalance.toFixed(2)} <span className="text-2xl font-bold opacity-80">XLM</span>
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowMnemonic(!showMnemonic)} className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl font-bold text-xs text-slate-600 hover:bg-slate-100 transition-colors">
                {showMnemonic ? <><EyeOff className="w-3.5 h-3.5" /> Hide</> : <><Eye className="w-3.5 h-3.5" /> Reveal</>}
              </button>
              <button type="button" onClick={copyMnemonic} className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl font-bold text-xs text-slate-600 hover:bg-slate-100 transition-colors">
                {mnemonicCopied ? <><Check className="w-3.5 h-3.5 text-green-600" /> Copied!</> : <><Copy className="w-3.5 h-3.5" /> Copy</>}
              </button>
            </div>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {wallet.mnemonic.split(" ").map((word, i) => (
              <div key={`ready-word-${word}-${i}`} className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 flex flex-col items-center gap-1">
...
function RestoreWalletView({
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full mt-12 lg:mt-24 flex-1">
      <button type="button" onClick={onBack} className="absolute top-24 left-6 lg:left-12 flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold text-sm bg-white border border-slate-200 px-4 py-2 rounded-full shadow-sm transition-colors">
...
export default function WalletPage() {
  ...
  const hasRealGoogleClientId = Boolean(
    ...
    process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID !== "demo-client-id.apps.googleusercontent.com" &&
  ...
  const googleLoginTrigger = useGoogleLogin({
    ...
    onError: () => setError("Google sign-in failed. Please use Instant Local Wallet or a Web3 Wallet."),
  });

  const handleGoogleLogin = () => {
    if (!hasRealGoogleClientId) {
      setError("Google OAuth is not configured. Please set NEXT_PUBLIC_GOOGLE_CLIENT_ID in your .env or click 'Create Instant Local Wallet'.");
      return;
    }
    googleLoginTrigger();
  };
          ...
          <div className="flex flex-col lg:flex-row items-center lg:items-start justify-between w-full mt-8 lg:mt-12 flex-1 gap-12 lg:gap-0">
            <div className="flex flex-col items-start w-full lg:w-5/12 z-10 lg:pt-16">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-14 w-full max-w-[540px]">
                {FEATURES.map((tag) => (
                  <div key={tag.label} className="bg-green-50/60 border border-green-100 rounded-2xl p-3.5 flex flex-col gap-2.5 shadow-sm hover:-translate-y-0.5 transition-transform">
                    {tag.icon}
          ...
          <div className="w-full max-w-[1400px] mx-auto px-6 lg:px-12 mb-10">
            <div className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] p-8 lg:p-10 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.04)] border border-white flex flex-col xl:flex-row items-center gap-12 relative overflow-hidden">
              ...
              <div className="flex-1 flex flex-col md:flex-row items-center justify-between gap-6 w-full z-10">
                {STEPS.map((s, i) => {
                  const stepCardBg = s.color === "blue" ? "bg-white border-slate-100" : `bg-${s.color}-50/50 border-${s.color}-100`;
                  const stepBadgeBg = `bg-${s.color}-50 text-${s.color}-600 border-${s.color}-100`;

                  return (
                    <div key={`step-${s.step}`} className="flex flex-col items-center flex-1 w-full relative group">
                      <div className={`w-6 h-6 rounded-full ${stepBadgeBg} font-extrabold text-[11px] flex items-center justify-center absolute -top-3 sm:left-1/2 sm:-ml-[32px] xl:left-14 border shadow-sm z-20`}>{s.step}</div>
                      <div className={`w-16 h-16 rounded-2xl ${stepCardBg} shadow-sm border flex items-center justify-center mb-5 z-10 group-hover:-translate-y-1 transition-transform`}>
                        {s.icon}
                      </div>
