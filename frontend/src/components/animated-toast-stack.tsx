...
const ToastItem = memo(function ToastItem({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative overflow-hidden rounded-2xl border border-slate-100 bg-white/95 p-3 shadow-2xl backdrop-blur-xl",
          classNames?.surface,
        )}
      >
        {renderToast ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className={cn("min-w-0 flex-1", classNames?.content)}>
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <p
                    className={cn(
                      "truncate text-sm font-bold leading-5 text-slate-800",
                      classNames?.title,
                    )}
                  >
                    {toast.title}
                  ...
                  {toast.description ? (
                    <p
                      className={cn(
                        "mt-0.5 line-clamp-2 text-xs font-semibold leading-4 text-slate-500",
              ...
              {toast.action ? (
                <button
                  ...
                  className={cn(
                    "mt-2 inline-flex h-7 items-center rounded-full bg-slate-100 px-3 text-xs font-bold text-slate-700 transition-colors hover:bg-slate-200",
                    classNames?.action,
                  )}
                >
                  {toast.action.label}
            ...
            {canDismiss ? (
              <button
                ...
                className={cn(
                  "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700",
                  classNames?.close,
                )}
              >
