...
import { Landmark, Gift, Wallet, TextCursorInput, ArrowRight, ArrowLeft, Check, Loader2, ExternalLink } from "lucide-react";
...
export default function WithdrawCard({ publicKey, onBack, onSent }: WithdrawCardProps) {
  const [mode, setMode] = useState<"menu" | "address">("menu");
  ...
  const menuRow = (icon: React.ReactNode, title: React.ReactNode, desc: string, onClick: () => void, extra?: React.ReactNode) => (
    <div className="group w-full overflow-hidden">
      <div onClick={onClick} className="flex w-full cursor-pointer items-center justify-start whitespace-normal break-words px-4 py-3.5">
        ...
        {extra ?? <ArrowRight className="h-5 w-5 flex-shrink-0 text-grey-800" />}
      </div>
    </div>
  );
    ...
    return (
      <div className="text-center">
        <div className="flex flex-col items-center py-10">
          ...
          <h3 className="mb-2 text-2xl font-bold text-grey-800">Withdrawal Successful</h3>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => { setMode("menu"); setStatus("idle"); setAmount(""); setDestination(""); }} className="flex h-11 items-center gap-2 rounded-lg bg-primaryBlue px-4 font-bold text-white transition-colors hover:bg-blue-600 active:bg-blue-700">Withdraw Again</button>
    ...
    return (
      <div className="text-center">
        <div className="flex-col">
          <h4 className="mb-3 flex w-full items-center justify-start text-left text-lg font-bold text-grey-800 mobile:text-[26px]">Withdraw</h4>
          <p className="mb-4 text-left text-sm font-normal text-grey-700 mobile:text-base">Select destination for withdrawal:</p>

          <div className="mt-4 overflow-hidden rounded-lg border border-grey-100 bg-white">
            {menuRow(
              <Landmark className="h-[23px] w-[23px]" />,
              <span className="inline-flex items-start">To Bank Account</span>,
              "Withdraw to a bank account you specify (US and Europe only). Powered by Coinflow.",
              () => setMode("address")
            )}
            <hr className="m-0 border-t border-grey-100" />
            {menuRow(
              <Gift className="h-5 w-5 flex-shrink-0" />,
              <p className="flex flex-row items-center gap-1">Gift Card</p>,
              "Buy gift cards from various brands with crypto! Powered by Bidali.",
              () => setMode("address")
            )}
          </div>

          <p className="mt-2 text-left text-xs text-grey-600">
            For any Bidali questions and support,{" "}
            <a href="https://support.spend.bidali.com/#contact" target="_blank" rel="noopener noreferrer" className="cursor-pointer font-semibold text-blue-500 hover:text-blue-600 active:text-blue-700">click here.</a>
          </p>
          ...
          <div className="mt-5 overflow-hidden rounded-lg border border-grey-100 bg-white">
            {menuRow(
              ...
              "To External Wallet",
              "Assets will be sent to a connected wallet",
              () => setMode("address")
            )}
            <hr className="m-0 border-t border-grey-100" />
            {menuRow(
              <TextCursorInput className="h-5 w-5" />,
              "To Stellar Wallet Address",
              "Assets will be sent to a Stellar wallet address you specify.",
              () => setMode("address")
            )}
          </div>
  ...
  return (
    <div className="text-center">
      <div className="flex-col">
        ...
        <h4 className="mb-3 flex w-full items-center justify-start text-left text-lg font-bold text-grey-800 mobile:text-[26px]">Withdraw to Stellar Wallet Address</h4>
        <p className="mb-4 text-left text-sm font-normal text-grey-700 mobile:text-base">Assets will be sent to a Stellar wallet address you specify:</p>
          ...
          <div className="flex w-full">
            {[1, 2, 5].map(v => (
              <button
                ...
                className="flex w-full items-center justify-center border-b border-r border-grey-100 bg-white py-2 first:rounded-bl-lg first:border-l last:rounded-br-lg hover:bg-grey-25 active:bg-grey-50"
              >
                ${v}
              </button>
            ))}
        ...
        <div className="flex w-full items-center justify-between gap-2 pt-[15px]">
          ...
          <div className="w-full mobile:w-max">
            ...
            >
              ...
              <span className="font-bold">{status === "sending" ? "Processing..." : "Confirm Withdraw"}</span>
            </button>
          </div>
</div>
        </div>
