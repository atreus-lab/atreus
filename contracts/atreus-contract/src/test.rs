...
use soroban_sdk::{
    ...
    token::{Client as TokenClient, StellarAssetClient}, Address, Bytes, BytesN, Env, IntoVal,
    Symbol,
};
...
fn test_email_restricted_claim() {
    ...
    client.claim_link(&link_hash, &recipient, &secret, &intended_hash, &relayer, &0i128);
}
...
fn test_create_and_claim() {
    ...
    let recipient = Address::generate(&env);
    let relayer = Address::generate(&env);
    client.claim_link(&link_hash, &recipient, &secret, &empty_email_hash(&env), &relayer, &0i128);
}
...
fn test_claim_pays_fee_bound_relayer_and_remainder_to_recipient() {
    ...
    // This mock represents the user's Soroban authorization entry. Its arguments
    // include the relayer and fee, just as the signed claim transaction does.
    client
        .mock_auths(&[MockAuth {
            address: &recipient,
...
fn test_claim_rejects_tampered_fee_not_covered_by_user_authorization() {
    ...
    // The authorization is valid only for `signed_fee`. Changing the fee in the
    // submitted invocation makes it fail at recipient.require_auth(), which is
    // the same result an altered XDR has after the user has signed it.
    assert!(client
        .mock_auths(&[MockAuth {
            address: &recipient,
            invoke: &MockAuthInvoke {
...
fn test_wrong_secret_fails() {
    ...
    let recipient = Address::generate(&env);
    let relayer = Address::generate(&env);
    assert!(client.try_claim_link(&link_hash, &recipient, &wrong_secret, &empty_email_hash(&env), &relayer, &0i128).is_err());
}
...
fn test_double_claim_fails() {
    ...
    assert!(client.try_claim_link(&link_hash, &recipient, &secret, &empty_email_hash(&env), &relayer, &0i128).is_err());
}
